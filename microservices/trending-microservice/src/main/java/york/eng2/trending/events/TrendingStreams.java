...
public class TrendingStreams {
	...
	/*
	 * @Singleton public KStream<WindowedIdentifier, Long>
	 * likedByDay(ConfiguredStreamBuilder builder) { Properties props =
	 * builder.getConfiguration(); props.put(StreamsConfig.APPLICATION_ID_CONFIG,
	 * "videos-metrics");
	 * 
	 * 
	 ...
	 * )) .count(Materialized.as("most-liked-by-hour")).toStream() .selectKey((k, v)
	 * -> new WindowedIdentifier(k.key(), k.window().start(), k.window().end()));
	 * 
	 * stream.to(TOPIC_MOST_LIKED_BY_HOUR,
	 * Produced.with(serdeRegistry.getSerde(WindowedIdentifier.class),
	 * Serdes.Long()));
	 * 
	 * return stream; }
	 */
